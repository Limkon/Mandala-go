...
// HandshakeSocks5 实现了 SOCKS5 客户端握手协议
// 修复点：严格控制读取字节数，防止读取后续业务数据造成断流
func HandshakeSocks5(conn net.Conn, user, pass, host string, port int) error {
	// 1. 发送版本和支持的认证方法
	// 如果有用户名密码，发送 0x02 (Username/Password)
	// 否则发送 0x00 (No Authentication)
	if user != "" && pass != "" {
	...
	// 这里是之前最容易出错的地方。我们需要先读头部，判断长度，再读剩余部分。
	// 绝对不能多读，否则会吃掉真正的网页数据。
